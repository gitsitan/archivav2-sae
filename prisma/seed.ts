...
// Load permissions JSON at runtime to avoid TS JSON module resolution issues
const permissionsRaw = fs.readFileSync(
  path.join(__dirname, "data", "permissions.json"),
  "utf8"
);
const permissionsData = JSON.parse(permissionsRaw) as Array<{ id: string }>;
const allPermissionIds = permissionsData.map((p: { id: string }) => p.id);
...
async function seedUsers() {
  ...
  for (const userData of users) {
    ...
    if (!existingUser) {
      // cast to any because seed script may not include all required fields in dev
  ...
  const groups = [
    {
      ...
      // Assign all permission IDs from the permissions JSON to the Super Administrateurs group
  ...
  for (const relation of userGroupRelations) {
    ...
    if (user && group) {
      ...
      if (!existingRelation) {
        ...
        console.log(
          `✅ User-Group relation created: ${user.email} -> ${group.name}`
        );
      } else {
        console.log(
          `⚠️  User-Group relation already exists: ${user.email} -> ${group.name}`
        );
      }
    } else {
      console.log(
        `❌ Could not find user or group for relation: ${relation.userEmail} -> ${relation.groupName}`
      );
    }
  }
}
